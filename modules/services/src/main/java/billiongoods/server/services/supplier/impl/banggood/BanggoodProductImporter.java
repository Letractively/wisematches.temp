...
public class BanggoodProductImporter implements ProductImporter {
	...
	private void importProduct(SuppliedProduct supplied, Set<String> images, boolean validatePrice) {
		...
		try {
			if (productManager.searchBySku(supplied.sku) == null) {
				...
				editor.setStockInfo(new StockInfo());
	...
	protected List<SuppliedProduct> parseProducts(InputStream stream) throws IOException {
		...
		String[] row = reader.readNext(); // ignore header
		while ((row = reader.readNext()) != null) {
			int index = 0;
			final String sku = row[index++];
			try {
				String name = ""; // build name. In this case name always contains category and sub-category
				while (!isDouble(row[index])) {
					name += ", " + row[index++];
				}

				final double price = Double.parseDouble(row[index++]);
				final double weight = Double.parseDouble(row[index++]);

				String desc = cleanSpan(row[index++]);

				String urlCell = "";
				while (row[index] != null && !row[index].endsWith(".html")) {
					urlCell += row[index++] + ",";
				}
				urlCell += row[index]; // and last part of URL

				URL url = new URL(urlCell);
				res.add(new SuppliedProduct(sku, url, url.getFile(), name, desc, price, weight));
			} catch (NumberFormatException ex) {
	...
	static class SuppliedProduct implements SupplierInfo {
