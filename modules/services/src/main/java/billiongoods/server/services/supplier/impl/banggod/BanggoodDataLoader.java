...
//		cookieStore.addCookie(new BasicClientCookie("_bgNoCache", "1"));
//		cookieStore.addCookie(new BasicClientCookie("js_shipping_country_254686", "176"));
	}
...
//		return new DefaultSupplierDescription(price, stockInfo, null);
	...
	protected Price loadStockPrice(SupplierInfo supplier) throws DataLoadingException {
		try {
			//	http://www.banggood.com/?action=&products_id=909947&wh=CN&=1&vids[]=

/*
			cookieStore.addCookie(new BasicClientCookie("js_shipping_country_254686", "176"));
			cookieStore.addCookie(new BasicClientCookie("header_defalt", "a%3A2%3A%7Bs%3A7%3A%22counrty%22%3Bi%3A176%3Bs%3A7%3A%22bizhong%22%3Bs%3A3%3A%22RUB%22%3B%7D"));
			cookieStore.addCookie(new BasicClientCookie("bg_registered_cus", "1"));
			cookieStore.addCookie(new BasicClientCookie("osCsid", "ffnrc47u17af108q5lb7kbjot0"));
			cookieStore.addCookie(new BasicClientCookie("_zcDataMagicUv", "1"));
			cookieStore.addCookie(new BasicClientCookie("osCsid", "ffnrc47u17af108q5lb7kbjot0"));
			cookieStore.addCookie(new BasicClientCookie("customer_view_products", "a%3A1%3A%7Bi%3A0%3Bi%3A39473%3B%7D"));
			cookieStore.addCookie(new BasicClientCookie("_bgNoCache", "1"));
			cookieStore.addCookie(new BasicClientCookie("_bgCK", "05780a7fd9f888f441e74d4a30239c41"));
*/

			final HttpPost request = new HttpPost("/ajax_cart.php?action=shipping&products_id=916095&wh=CN&oids[]=49&vids[]=1968");
			request.setHeader("Origin", supplier.getWholesaler().getSite());
			request.setHeader("Referer", supplier.getReferenceUrl().toString());
			request.setHeader("contentType", "application/x-www-form-urlencoded; charset=UTF-8");

			final List<NameValuePair> nvps = new ArrayList<>();
			nvps.add(new BasicNameValuePair("action", "shipping"));
			nvps.add(new BasicNameValuePair("wh", "CN"));
			nvps.add(new BasicNameValuePair("products_id", "916095")); //supplier.getReferenceId()
			nvps.add(new BasicNameValuePair("oids[]", "49"));
			nvps.add(new BasicNameValuePair("vids[]", "1968"));
...
//				return parsePrice(split[1]);
//			}
			if (true) {
				return null;
			}
			throw new DataLoadingException("Price response is not correct: " + s);
		} catch (DataLoadingException ex) {
			throw ex;
		} catch (Exception ex) {
			throw new DataLoadingException("Price response  - " + ex.getMessage(), ex);
		}
	}
