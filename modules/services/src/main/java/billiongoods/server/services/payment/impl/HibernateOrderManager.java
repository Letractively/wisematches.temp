...
public class HibernateOrderManager extends EntitySearchManager<Order, OrderContext, Void> implements OrderManager {
	...
	@Override
	@Transactional(propagation = Propagation.MANDATORY)
	public Order refund(Long orderId, String token, double amount, String note) {
		final Session session = sessionFactory.getCurrentSession();

		final HibernateOrder order = getOrder(orderId);
		order.refund(token, amount, note);
		session.update(order);

		for (OrderListener orderListener : orderListeners) {
			orderListener.orderRefund(order, token, amount);
		}
		return order;
	}
	...
	@Override
	@Transactional(propagation = Propagation.MANDATORY)
	public Order modify(Long orderId, OrderItemChange... changes) {
		final Session session = sessionFactory.getCurrentSession();

		final HibernateOrder order = getOrder(orderId);
		order.modify(changes);
		session.update(order);

		for (OrderListener orderListener : orderListeners) {
			orderListener.orderContentChanged(order, changes);
		}
		return order;
	}
	...
	@Override
	@Transactional(propagation = Propagation.MANDATORY)
	public Order failed(Long orderId, String errorCode, String reason) {
		final Session session = sessionFactory.getCurrentSession();

		final HibernateOrder order = getOrder(orderId);
		final OrderState state = order.getState();
		order.failed(errorCode, reason);
		session.update(order);
	...
	@Override
	@Transactional(propagation = Propagation.MANDATORY)
	public Order failed(String token, String errorCode, String reason) {
		final Session session = sessionFactory.getCurrentSession();
