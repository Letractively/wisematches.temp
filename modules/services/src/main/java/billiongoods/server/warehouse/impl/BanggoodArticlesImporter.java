...
public class BanggoodArticlesImporter {
    ...
    public void importArticles(Category category, InputStream stream) throws IOException {
        ...
        while ((nextLine = reader.readNext()) != null) {
            ...
            final float price = Float.parseFloat(nextLine[3]);
            final String desc = nextLine[5];

            String id = nextLine[6];
            id = id.substring(id.lastIndexOf("-") + 1, id.lastIndexOf("."));

            final float price1 = Math.round((price + price * 0.15f + 0.30f) * 100f) / 100.0f;

            articleManager.createArticle(name, desc, category, price1, null, null, null, null, null, null, null, id, sku, Supplier.BANGGOOD, price, null);
        }
    }
