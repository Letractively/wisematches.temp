...
public class PayPalController extends AbstractController {
    ...
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public String orderAccepted(@RequestParam("token") String token, HttpSession session) {
        return processOrderState(token, session, true);
    }

    @RequestMapping("/rejected")
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public String orderRejected(@RequestParam("token") String token, HttpSession session) {
        return processOrderState(token, session, false);
    }

    private String processOrderState(String token, HttpSession session, boolean accepted) {
        try {
            ...
            session.setAttribute("orderId", transaction.getOrderId());
            session.setAttribute("orderToken", transaction.getId());
        } catch (PayPalException ex) {
